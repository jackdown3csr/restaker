...
### Added
- New **gUBI tab** — shows rank, SoulScore, monthly reward, share, pool composition (via Galactica API)
- Periodic update checker every 6 hours (not just on startup)

## [1.0.5] - 2026-02-12

### Fixed
- No longer auto-extends on startup — first extend waits for scheduled interval
- User can explore the app and settings before any transaction is sent

## [1.0.4] - 2026-02-11

### Added
- Live countdown "Next extend in Xh Ym" in footer

### Fixed
- Footer (version + countdown) was invisible — clipped by notebook; now packed with bottom priority

## [1.0.3] - 2026-02-11

### Added
- Update checker — notifies when a new version is available on GitHub
- Version number shown in GUI footer

### Fixed
- Autostart now uses Task Scheduler instead of registry Run key (bypasses SmartScreen)
- Double notification on startup (pystray + winotify) — now only winotify with app icon

## [1.0.2] - 2026-02-10

### Added
- Vesting rewards checker toggle in GUI settings
- Separate vesting check interval (independent of auto-extend)
- Vesting status on dashboard (status, epochs behind, total claimed, last check)
- Manual "Check Vesting Now" button
- "Start on Windows login" option (writes to `HKCU\...\Run`)

### Changed
- GUI layout adjusted to prevent clipped content

## [1.0.1] - 2026-02-10

### Added
- Application icon (LOGO_PNG.png)
- Toast notifications with app icon (winotify)
